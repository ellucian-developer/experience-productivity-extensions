...
export function initializeGraphClient({aadClientId, aadTenantId, cacheGetItem, cacheStoreItem, msalClient, setError}) {
    const options = {
        authProvider: {
            getAccessToken: () => (acquireToken({aadClientId, aadTenantId, cacheGetItem, cacheStoreItem, msalClient, setError}))
        }
    }
...
export async function acquireToken({aadClientId, aadTenantId, cacheGetItem, cacheStoreItem, msalClient, setError, trySsoSilent = false}) {
    if (msalClient) {
        let account = msalClient.getActiveAccount();

        if (!account) {
            // see if it is saved in cache
            const { data: { lastMSUsername } = {} } = cacheGetItem({...lastLoginCacheOptions});
            if (lastMSUsername) {
